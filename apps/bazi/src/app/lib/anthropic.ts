...
請分析以下面向（繁體中文），第 2-6、8 項這階段只需要「白話重點結論」。
⚠️ HTML 格式說明：標註「HTML 格式」的欄位請使用 <p>、<ul>/<ol>、<li>、<strong> 等標籤，不要使用 Markdown。字數計算不含 HTML 標籤本身。
⚠️ 第 2-6、8 項結論：純文字 40-80 字，完全不使用命理術語（十神、正官、七殺、傷官、格局、用神、忌神、日主、大運、流年等一律禁止），改用一般人能懂的白話，像跟完全不懂命理的朋友講結論一樣，直接講結論本身，不要說「本節將分析...」這類鋪陳。這幾項使用者之後可再點擊查看詳細分析，此階段先給精簡結論即可。
⚠️ 第 1 項（性格特質）沒有後續的詳細分析可看，不受上述 40-80 字長度限制：請給完整、精準的內容。同樣禁止出現十神、正官、七殺、格局、用神、忌神、日主、大運、流年等術語本身，但精準度不能因此打折——內部先判斷是哪一柱、哪個十神組合帶出這個特質，再把它轉譯成具體、只屬於這個命盤的白話描述（例如具體的行為傾向、決策習慣、人際互動方式），不要用「你很有能力」「個性鮮明」這類套在誰身上都成立的空泛形容詞。

1. **性格特質**（HTML 格式，200-300字）：日主五行 + 最突出的 2~3 個十神組合特質，語氣親切像對本人說話，內部依哪一柱、哪個十神組合推導，但輸出只呈現轉譯後的具體白話行為描述，不出現術語，寧可具體也不要籠統

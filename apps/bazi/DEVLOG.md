...
- [系統架構](#系統架構)
  - [使用者算命流程](#使用者算命流程)
  - [AI Token 使用點](#ai-token-使用點)
- [架構設計思考](#架構設計思考)
  - [同命盤快取 vs 個人化記錄](#同命盤快取-vs-個人化記錄)
- [環境變數說明](#環境變數說明)
- [遇到的問題、取捨與解法](#遇到的問題、取捨與解法)
  - [0. 資料庫從 Prisma 改為 Firebase](#0-資料庫從-prisma-改為-firebase)
  - [1. `entries.map is not a function` 崩潰](#1-entriesmap-is-not-a-function-崩潰)
  - [2. Firestore API 未啟用（500 根本原因）](#2-firestore-api-未啟用（500-根本原因）)
  - [3. Favicon 無法顯示正確樣式](#3-favicon-無法顯示正確樣式)
  - [4. YouTube 字幕抓取方案選型](#4-youtube-字幕抓取方案選型)
  - [5. 知識庫全量注入 → 標籤式 RAG](#5-知識庫全量注入-→-標籤式-rag)
  - [6. Anthropic API 無免費方案，換用 Google Gemini](#6-anthropic-api-無免費方案，換用-google-gemini)
  - [7. Firebase Admin SDK 在 Next.js build 時初始化失敗](#7-firebase-admin-sdk-在-nextjs-build-時初始化失敗)
  - [8. readingId 分散在元件，導致重複呼叫與重導邏輯混亂](#8-readingid-分散在元件，導致重複呼叫與重導邏輯混亂)
  - [9. Vercel 部署後所有需要 Firebase Auth 的 API 一律回 403](#9-vercel-部署後所有需要-firebase-auth-的-api-一律回-403)
  - [10. TanStack Query 整合取代手動 fetch 狀態管理](#10-tanstack-query-整合取代手動-fetch-狀態管理)
  - [11. 目錄結構調整：`components/` → `common/`](#11-目錄結構調整：components-→-common)
  - [12. `patchReadings` 共用快取 helper](#12-patchreadings-共用快取-helper)
  - [13. TypeScript control-flow narrowing 造成 `never` 型別錯誤](#13-typescript-control-flow-narrowing-造成-never-型別錯誤)
  - [14. 參數命名規範化：禁用單字元參數名稱](#14-參數命名規範化：禁用單字元參數名稱)
  - [15. API Response 減量與 Cache-Control：避免個資外洩](#15-api-response-減量與-cache-control：避免個資外洩)
  - [16. Google OAuth 403：LINE 內建瀏覽器封鎖登入](#16-google-oauth-403：line-內建瀏覽器封鎖登入)
  - [17. 天生卡貓咪圖片：轉 WebP + 長效快取，減少重複下載](#17-天生卡貓咪圖片：轉-webp--長效快取，減少重複下載)
  - [18. 一人一次命盤限制忽略修正資料，導致填錯重填後仍看到舊結果](#18-一人一次命盤限制忽略修正資料，導致填錯重填後仍看到舊結果)
  - [19. Firestore 讀寫散落在 9 個 route.ts，改成 Repository + Service 分層](#19-firestore-讀寫散落在-9-個-routets，改成-repository--service-分層)
  - [20. 建立 Vitest 測試框架：Node 版本相容性踩雷](#20-建立-vitest-測試框架：node-版本相容性踩雷)
- [待辦事項](#待辦事項)
...
學了線上課程後發現八字規則太多背不起來，實戰時常卡關。於是把「知識衛星」40 支教學影片的字幕爬出來，先讓 AI 整理成筆記，再放進系統裡持續擴充功能。

做成網站而非單純問 AI，是希望朋友也能用：綁定 Google 帳號給每人專屬空間、每日 3 題輸入框自行提問。上線後意外成為朋友間的話題，也讓我對命盤規則更熟練、更懂朋友的個性。
